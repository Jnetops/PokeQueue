$(document).ready(function() {
  ...
  var pusher = new Pusher(appKey, { cluster: 'us2', authEndpoint: '/events/'+eventID+'/chat/auth', auth: { headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }}});
  Pusher.log = function(msg) {
    console.log(msg);
  };
  var channelSub = pusher.subscribe('private-event-channel-' + eventID);
  channelSub.bind('App\\Events\\newMessage', function(data) {
    $(".chat-list-event").append('<li class="li-event-item"><div class="col-md-12 chat-col"><div class="col-md-1 text-center"><div class="profile-avatar tiny text-center" style="background-image: url('+data.text.avatar+')"></div></div><div class="col-md-9"><a class="trainer-name" href="/users/"'+data.text.trainer+'>'+data.text.trainer+'</a><p>'+data.text.text+'</p></div><div class="col-md-2"><p>'+data.text.timestamp+'</p></div></div></li>');
    $("#event-chat-comment").val('');
